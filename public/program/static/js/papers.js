...
const card_image = (openreview, show) => {
    if (show) return ` <center style="flex-grow: 1;"><img class="lazy-load-img cards_img" data-src="static/paper_images/${openreview.id}" data-fallback-src="static/paper_images/${openreview.id}.png" onerror="if (!this.dataset.fallbackTried) { this.dataset.fallbackTried = '1'; this.src = this.dataset.fallbackSrc; } else { this.style.display = 'none'; }" width="80%"/></center>`
    else return ''
}
